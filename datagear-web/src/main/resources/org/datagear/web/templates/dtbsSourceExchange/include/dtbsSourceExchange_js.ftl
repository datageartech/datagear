...
(function(po)
{
	...
	{
		po.open("/dtbsSourceExchange/" + encodeURIComponent(po.dtbsSourceId) +"/viewLog",
		{
			data :
			{
	...
	po.cancelSubDataExchanges = function()
	{
		...
		{
			po.ajaxJson("/dtbsSourceExchange/" + encodeURIComponent(po.dtbsSourceId) +"/cancel",
			{
				tipSuccess: false,
				data:
				{
	...
	po.setupDataExchange = function()
	{
		...
		po.dataExchangeTaskClient = new $.TaskClient(
				po.concatContextPath("/dtbsSourceExchange/"+encodeURIComponent(po.dtbsSourceId)+"/message"),
				function(message)
				{
