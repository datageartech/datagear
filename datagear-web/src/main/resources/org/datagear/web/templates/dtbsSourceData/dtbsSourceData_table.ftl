...
<div id="${pid}" class="page page-manager page-table page-dtbsSourcedata h-full flex flex-column overflow-auto">
	<div class="page-header grid grid-nogutter align-items-center p-1 flex-grow-0">
		...
		<div class="col-12" :class="pm.isSelectAction ? 'md:col-6' : 'md:col-4'">
			<#include "include/dtbsSourceData_search_form.ftl">
		</div>
		...
		po.vueMethod(
		{
			...
			onExport: function()
			{
				...
				po.ajaxJson(po.dataUrl("getQuerySql"),
				{
					...
					{
						var url = "/dtbsSourceExchange/"+encodeURIComponent(po.dtbsSourceId)+"/export";
