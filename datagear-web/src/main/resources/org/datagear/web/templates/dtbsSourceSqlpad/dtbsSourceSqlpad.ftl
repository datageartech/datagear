...
<div id="${pid}" class="page page-manager page-sqlpad h-full flex flex-column overflow-auto">
	<div class="page-header grid grid-nogutter align-items-center p-1 flex-grow-0">
		<div class="col-12 flex align-items-center mb-2">
			...
			<div class="text-color-secondary text-sm ml-1">${dtbsSource.title}</div>
...
(function(po)
{
	...
	po.dtbsSourceId = formModel.dtbsSourceId;
	po.sqlpadId = formModel.sqlpadId;
	po.submitUrl = "/dtbsSourceSqlpad/"+encodeURIComponent(po.dtbsSourceId)+"/execute";
	
	po.getSqlEditorDtbsSourceId = function()
	{
		return po.dtbsSourceId;
	};
	...
	po.executeSql = function(sql, sqlStartRow, sqlStartColumn)
	{
		...
		po.ajax("/dtbsSourceSqlpad/"+encodeURIComponent(po.dtbsSourceId)+"/execute",
		{
			tipSuccess: false,
			type : "POST",
			data : data,
	...
	po.sendSqlCommand = function(sqlCommand)
	{
		...
		po.ajax("/dtbsSourceSqlpad/"+encodeURIComponent(po.dtbsSourceId)+"/command",
		{
			tipSuccess: false,
			type: "POST",
			data:
	...
	po.loadMoreSqlResult = function(tab)
	{
		...
		result.loading = true;
		po.ajaxJson("/dtbsSourceSqlpad/"+encodeURIComponent(po.dtbsSourceId)+"/select",
		{
			data:
			{
				sqlpadId: po.sqlpadId,
	...
	{
		po.openTableDialog("/dtbsSourceExchange/"+encodeURIComponent(po.dtbsSourceId)+"/export?query="+encodeURIComponent(tab.sql || ""));
	};
	...
	po.loadSqlHistory = function()
	{
		...
		if(wrapper.length > 0)
		{
			...
			po.open("/dtbsSourceSqlpad/"+encodeURIComponent(po.dtbsSourceId)+"/sqlHistory?multiple",
			{
				target: wrapper,
				dialog: false,
				pageParam:
	...
	po.vueMethod(
	{
		...
		onRenderColumnValue: function(column, slotProps)
		{
			...
			if($.tableMeta.isBinaryColumn(column))
			{
				...
				{
					var downloadUrl = po.concatContextPath("/dtbsSourceSqlpad/"+encodeURIComponent(po.dtbsSourceId)+"/downloadResultField");
	...
	po.vueMounted(function()
	{
		...
		po.sqlpadTaskClient = new $.TaskClient(po.concatContextPath("/dtbsSourceSqlpad/"+encodeURIComponent(po.dtbsSourceId)+"/message"),
		function(message)
		{
