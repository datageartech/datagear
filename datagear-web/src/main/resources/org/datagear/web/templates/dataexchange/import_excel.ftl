				...
				<div>
					<div class="form-item form-item-table-head form-item-upload page-status-aware-show edit-status-show">
						<div class="uploadFileWrapper form-item-value">
							...
							-->
							<div class="fileinput-button button" upload-action="excel/uploadImportFile" title="<@spring.message code='dataImport.uploadExcelDataFile.desc' />"><@spring.message code='add' /><input type="file"></div>
...
(function(po)
{
	...
	po.dataImportTableColumns.splice(3, 0,
	{
		...
		{
			return "<input type='text' name='tableNames[]' value='"+$.escapeHtml(data)+"' placeholder='<@spring.message code='dataImport.excel.importTableInput.placeholder' />' class='table-name-input ui-widget ui-widget-content ui-corner-all' style='width:90%' />";
		},
		defaultContent: "",
		width : "20%"
	});
