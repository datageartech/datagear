...
(function(global, window)
{
	...
	dashboardFactory.init = function(dashboard)
	{
		...
		chartFactory.extend(dashboard, this.dashboardBase);
	...
	chartBase.links = function(links)
	{
		...
		{
			if(links && !chartFactory.isArray(links))
				links = [ links ];
	...
	chartBase._resolveLinkTriggers = function(link)
	{
		...
		if(!chartFactory.isArray(triggers))
			triggers = [ triggers ];
	...
	dashboardBase._initMapURLs = function()
	{
		...
		if(mapURLsBody)
			mapURLs = chartFactory.extend(mapURLs, chartFactory.evalSilently(mapURLsBody, {}));
	...
	dashboardBase.mapURLs = function(mapURLs)
	{
		...
		chartFactory.extend(chartFactory.chartMapURLs, mapURLs);
	};
	...
	dashboardBase.renderForm = function(form, config)
	{
		...
		if(config.link)
		{
			...
			//转换简写格式
			if(chartFactory.isString(link) || chartFactory.isArray(link))
				link = { target: link };
			
			batchSet.target = link.target;
			//新构建data对象，因为可能会在下面被修改
			batchSet.data = (link.data ? chartFactory.extend({}, link.data) : {});
		}
		...
		if(!chartFactory.isArray(sourceItems))
	...
	dashboardBase._buildDashboardQueryForm = function(chartQueryPairs)
	{
		...
		if(globalResultDataFormat)
			globalResultDataFormat = chartFactory.extend(true, {}, globalResultDataFormat);
	...
	dashboardBase._buildChartQuery = function(chart)
	{
		...
		{
			//这里需要深度拷贝，因为后续可能会被修改
			chartQuery.resultDataFormat = chartFactory.extend(true, {}, chartQuery.resultDataFormat);
		}
		else
		{
		...
		for(var i=0; i<dataSetBinds.length; i++)
		{
			...
			//这里需要深度拷贝，因为后续可能会被修改
			var dataSetQuery = chartFactory.extend(true, {}, dataSetBinds[i].query);
	...
	dashboardBase.loadChart = function(element, chartWidgetId, ajaxOptions)
	{
		...
		if(!ajaxOptions)
			ajaxOptions = {};
		else if(chartFactory.isFunction(ajaxOptions))
		{
		...
		var myAjaxOptions = chartFactory.extend({}, ajaxOptions);
	...
	dashboardBase.loadCharts = function(element, chartWidgetId, ajaxOptions)
	{
		...
		if(!chartFactory.isString(chartWidgetId) && !chartFactory.isArray(chartWidgetId))
		{
			ajaxOptions = chartWidgetId;
			chartWidgetId = null;
		}
	...
	{
		var isFetchSingle = (!chartFactory.isArray(chartWidgetId));
		
		var chartWidgetIds = chartWidgetId;
		
		if(!chartFactory.isArray(chartWidgetIds))
		...
		var myAjaxOptions = chartFactory.extend(
		{
			url: url,
			data: data,
			type: "POST"
	...
	dashboardBase._batchSetDataSetParamValues = function(sourceData, batchSet, sourceValueContext)
	{
		...
		var targets = (batchSet.target == null ? [] : (chartFactory.isArray(batchSet.target) ? batchSet.target : [ batchSet.target ]));
		var targetCharts = [];
		...
		var hasGetValueFunc = chartFactory.isFunction(sourceData.getValue);
		
		var sourceValueContextArgs = [ "place-holder-for-source-value" ];
		sourceValueContextArgs = sourceValueContextArgs.concat(chartFactory.isArray(sourceValueContext) ? sourceValueContext : [ sourceValueContext ]);
		...
		for(var name in dataMap)
		{
			...
			if(!chartFactory.isArray(indexes))
				indexes = [ indexes ];
	...
	dashboardBase.originalDataIndex = function(data, inflate)
	{
		...
		if(odIdx == null && chartFactory.isArray(data))
		{
			odIdx = [];
		...
		var isArray = chartFactory.isArray(odIdx);
		var odIdxAry = (isArray ? odIdx : [ odIdx ]);
		var odIdxAryClone = [];
		
		for(var i=0; i<odIdxAry.length; i++)
		{
			var odIdxMy = (odIdxAry[i] ? chartFactory.extend({}, odIdxAry[i]) : null);
