 ...
 * @param dataSign 与this.pluginDataSignNested()函数的identity参数相同，为null表示筛选无任何标记的数据集字段
 * @return 数据集字段、null
 */
...
chartProto._dataSetFieldsOfSign = function(dataSetBind, fieldsOwner, dataSign, count)
{
	...
	var dataSignObj = this.pluginDataSignNested(dataSign);
...
chartProto.sortDataSetFields = function(dataSetBind, fields)
{
	...
	{
		let f = fields[i];
		fos[i] = { field: f, order: this.dataSetFieldOrder(dataSetBind, f), index: i };
	}
 ...
 * @returns
 */
chartProto.pluginForm = function()
{
	var plugin = this.plugin();
	return (plugin ? plugin.form : null);
};
 ...
 * @returns {}、null
 */
chartProto.pluginFormValue = function()
{
	var form = this.pluginForm();
	
	if(form == null)
		return null;
	
	if(CF.isRootPluginForm(form))
	{
		let re = {};
 ...
 * 主件数据集绑定的用途是绘制图表。
 * 
 * @param dataSign 可选，如果设置，表示按指定数据集标记筛选，与this.pluginDataSignNested()函数的identity参数相同，为null表示筛选无任何标记的数据集绑定
...
chartProto._dataSetBindsOf = function(count, attachment, dataSign)
{
	...
	var dataSignObj = (dataSign === false ? null : this.pluginDataSignNested(dataSign));
...
{
	owner = (owner === undefined ? this.plugin() : owner);
...
chartProto.pluginDataSign = function(identity, dataSigns)
{
	...
	return this._findDataSign(dataSigns, identity);
};
 ...
 * @returns 数据标记，没有找到则是null
 */
chartProto.pluginDataSignNested = function(identity, dataSigns)
{
	if(identity == null)
		return null;
	...
	if(!isArray)
	{
		...
		//全名
			...
			return this._findDataSignByFullname(dataSigns, identity);
	}
	else
	{
		let dataSign = null;
...
chartProto._findDataSignByFullname = function(dataSigns, fullname)
{
	if(dataSigns == null || fullname == null)
		return null;
	...
	for(let i=0; i<dataSigns.length; i++)
	{
		...
		if(dataSign && dataSign.fullname == fullname)
			return dataSign;
		
		if(dataSign.children != null)
		{
			let subDataSign = this._findDataSignByFullname(dataSign.children, fullname);
 ...
 * 
 * @param dataSetBind 数据集绑定或其索引
 * @param dataSign 与this.pluginDataSignNested()函数的identity参数相同，为null表示匹配无任何标记的
 * @returns true、false
 */
 ...
 */
CF.isRootPluginForm = function(pluginForm)
{
...
CF.arrayContentEquals = function(a, b)
{
	if (a === b)
		return true;
	
	if(!CF.isArray(a) || !CF.isArray(b))
		return false;
...
CF.findUnloadedBestLib = function(lib, renderContext, libPlugins, contextCharts, inputLibs)
{
	...
	if(bestLibInfo.lib !== lib && CF.isLibLoaded(bestLibInfo.lib))
		return false;
	
	var bestLib = bestLibInfo.lib;
	
	if(bestLibInfo.from == "global")
	{
...
CF.inflateCandidateLibInfoInGlobalLib = function(libInfos, baseLib)
{
	for(let i=0; i<CF.globalLibs.length; i++)
	{
		...
		{
			libInfos.push({ lib: lib, priority: CF.LIB_PRIORITY_GLOBAL_LIB, from: "global" });
		}
	}
};

CF.inflateCandidateLibInfoInLibPlugins = function(libInfos, baseLib, libPlugins)
{
	if(libPlugins == null)
		return;
	...
	{
		let plugin = libPlugins[i];
		CF.inflateCandidateLibInfoInPlugin(libInfos, baseLib, plugin, CF.LIB_PRIORITY_LIB_PLUGIN);
	}
};

CF.inflateCandidateLibInfoInContextCharts = function(libInfos, baseLib, contextCharts)
{
...
CF.inflateCandidateLibInfoInPlugin = function(libInfos, baseLib, plugin, priority)
{
	if(plugin == null)
		return;
	...
	rendererLib = (rendererLib == null ? [] : (CF.isArray(rendererLib) ? rendererLib : [ rendererLib ]));
	
	for(let i=0; i<rendererLib.length; i++)
	{
		let lib = rendererLib[i];
		
		if(CF.isCandidateLib(lib, baseLib))
		{
			libInfos.push({ lib: lib, priority: priority, from: "plugin", plugin: plugin });
		}
	}
};

CF.inflateCandidateLibInfoInCustomRenderer = function(libInfos, baseLib, renderer, priority)
{
	if(renderer == null)
		return;
	...
	for(let i=0; i<rendererLib.length; i++)
	{
		...
		if(CF.isCandidateLib(lib, baseLib))
		{
			libInfos.push({ lib: lib, priority: priority, from: "customRenderer", renderer: renderer });
		}
	}
};

CF.isCandidateLib = function(lib, baseLib)
{
