...
public class JdbcSupport
{
	 ...
	 *            自动生成主键标识，参考{@linkplain Statement#NO_GENERATED_KEYS}、{@linkplain Statement#RETURN_GENERATED_KEYS}
	 * @return
	 * @throws SQLException
	 */
	public SqlExecuteState execute(Connection cn, Sql sql, int autoGeneratedKeys) throws SQLException
	{
		...
		try
		{
			...
			if (hasParamValue)
			{
				PreparedStatement pst = cn.prepareStatement(sql.getSqlValue(), autoGeneratedKeys);
				st = pst;
				setParams = setParamValues(cn, pst, rawParams);
			...
			else
			{
				...
				resultSet = st.execute(sql.getSqlValue(), autoGeneratedKeys);
			}
				...
				if (genKeyCount == autoGeneratedNames.length)
				{
					while (genKeys.next())
					{
						...
						for (int i = 0; i < autoGeneratedNames.length; i++)
						{
							...
							Object value = getColumnValueExtract(cn, genKeys, (i + 1), autoGeneratedTypes[i]);
							row.put(name, value);
						}
	...
	@JDBCCompatiblity("某些驱动程序可能不支持ResultSet.getObject()方法，所以这里没有使用。"
			+ "另外，这里也没有使用列名作为参数，因为存在某些不支持列名的场景会报错（比如Oracle执行Statement.RETURN_GENERATED_KEYS的INSERT语句时，"
			+ "Statement.getGeneratedKeys()获取的结果集）")
	public Object getColumnValueExtract(Connection cn, ResultSet rs, int column, int sqlType) throws SQLException
	{
		Object value = null;
		...
		switch (sqlType)
		{
			...
			default:
			{
				value = getColumnValueExtractExt(cn, rs, column, sqlType);
				break;
			}
		}
	...
	protected Object getColumnValueForARRAY(Connection cn, ResultSet rs, int column) throws SQLException
	{
		...
		return checkGotWasNull(cn, rs, column, value);
	}
	 ...
	 * @throws SQLException
	 */
	protected Object getColumnValueForBIGINT(Connection cn, ResultSet rs, int column)
			throws SQLException
	{
		Object value = null;
	 ...
	 * @throws SQLException
	 */
	protected Object getColumnValueForDOUBLE(Connection cn, ResultSet rs, int column)
			throws SQLException
	{
		Object value = null;
	 ...
	 * @throws SQLException
	 */
	protected Object getColumnValueForFLOAT(Connection cn, ResultSet rs, int column)
			throws SQLException
	{
		Object value = null;
		...
		catch (SQLException e)
		{
			...
			Object bigValue = getColumnValueForDOUBLE(cn, rs, column);
			value = bigValue;
		}
	 ...
	 * @throws SQLException
	 */
	protected Object getColumnValueForINTEGER(Connection cn, ResultSet rs, int column)
			throws SQLException
	{
		Object value = null;
		...
		catch (SQLException e)
		{
			...
			Object bigValue = getColumnValueForBIGINT(cn, rs, column);
			value = bigValue;
		}
	 ...
	 * @throws SQLException
	 */
	protected Object getColumnValueForSMALLINT(Connection cn, ResultSet rs, int column)
			throws SQLException
	{
		Object value = null;
		...
		catch (SQLException e)
		{
			...
			Object bigValue = getColumnValueForINTEGER(cn, rs, column);
			value = bigValue;
		}
	 ...
	 * @throws SQLException
	 */
	protected Object getColumnValueForTINYINT(Connection cn, ResultSet rs, int column)
			throws SQLException
	{
		Object value = null;
		...
		catch (SQLException e)
		{
			...
			Object bigValue = getColumnValueForSMALLINT(cn, rs, column);
			value = bigValue;
		}
	 ...
	 * @throws SQLException
	 */
	protected Object checkGotWasNull(Connection cn, ResultSet rs, int column, Object gotValue) throws SQLException
	{
